...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	geeTestService := service.NewGeeTestService(settingService, geeTestVerifier)
	...
	authService := service.NewAuthService(client, userRepository, redeemCodeRepository, refreshTokenCache, configConfig, settingService, emailService, turnstileService, geeTestService, emailQueueService, promoService, subscriptionService)
	...
	groupCapacitySnapshotProjector := service.NewGroupCapacitySnapshotProjector()
	groupCapacitySnapshotProvider := service.ProvideGroupCapacitySnapshotProvider(accountRepository, groupRepository, groupCapacitySnapshotProjector)
	groupCapacityRuntimeProvider := service.ProvideGroupCapacityRuntimeProvider(concurrencyService, sessionLimitCache, rpmCache)
	groupCapacityService := service.ProvideGroupCapacityService(accountRepository, groupRepository, concurrencyService, sessionLimitCache, rpmCache, groupCapacitySnapshotProvider, groupCapacityRuntimeProvider)
	...
	accountUsageService := service.NewAccountUsageService(accountRepository, usageLogRepository, claudeUsageFetcher, geminiQuotaService, antigravityQuotaFetcher, usageCache, identityCache, tlsFingerprintProfileService, settingService)
	oAuthRefreshAPI := service.ProvideOAuthRefreshAPI(accountRepository, geminiTokenCache)
	geminiTokenProvider := service.ProvideGeminiTokenProvider(accountRepository, geminiTokenCache, geminiOAuthService, oAuthRefreshAPI)
	gatewayCache := repository.NewGatewayCache(redisClient)
	schedulerOutboxRepository := repository.NewSchedulerOutboxRepository(db)
	opsRealtimeCache := repository.NewOpsRealtimeCache(redisClient)
	schedulerSnapshotService := service.ProvideSchedulerSnapshotService(schedulerCache, schedulerOutboxRepository, accountRepository, groupRepository, configConfig, opsRealtimeCache, groupCapacitySnapshotProjector)
	...
	openAIOAuthHandler := admin.NewOpenAIOAuthHandler(openAIOAuthService, adminService, settingService)
	...
	openAIGatewayService := service.ProvideOpenAIGatewayService(accountRepository, usageLogRepository, usageBillingRepository, userRepository, userSubscriptionRepository, proxyRepository, proxyExitInfoProber, userGroupRateRepository, gatewayCache, configConfig, schedulerSnapshotService, concurrencyService, billingService, rateLimitService, billingCacheService, httpUpstream, deferredService, openAITokenProvider, modelPricingResolver, channelService, balanceNotifyService, opsRealtimeCache, settingService, accountUsageService)
	geminiMessagesCompatService := service.NewGeminiMessagesCompatService(accountRepository, groupRepository, gatewayCache, schedulerSnapshotService, geminiTokenProvider, rateLimitService, httpUpstream, antigravityGatewayService, configConfig)
	opsSystemLogSink := service.ProvideOpsSystemLogSink(opsRepository)
	opsService := service.ProvideOpsService(opsRepository, settingRepository, configConfig, accountRepository, userRepository, concurrencyService, gatewayService, openAIGatewayService, geminiMessagesCompatService, antigravityGatewayService, opsRealtimeCache, opsSystemLogSink)
