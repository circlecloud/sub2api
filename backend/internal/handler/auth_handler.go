...
func resolveCaptchaToken(primary, legacy string) string {
...
func (h *AuthHandler) Register(c *gin.Context) {
	...
	// 验证码验证（邮箱验证码注册场景避免重复校验一次性 token）
	captchaToken := resolveCaptchaToken(req.CaptchaToken, req.TurnstileToken)
	if err := h.authService.VerifyCaptchaForRegister(c.Request.Context(), captchaToken, ip.GetClientIP(c), req.VerifyCode); err != nil {
...
func (h *AuthHandler) SendVerifyCode(c *gin.Context) {
	...
	// 验证码验证
	captchaToken := resolveCaptchaToken(req.CaptchaToken, req.TurnstileToken)
	if err := h.authService.VerifyCaptcha(c.Request.Context(), captchaToken, ip.GetClientIP(c)); err != nil {
		response.ErrorFrom(c, err)
		return
	}
