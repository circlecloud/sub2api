...
func (s *AccountTestService) testOpenAIAccountConnection(c *gin.Context, account *Account, modelID string) error {
	...
	upstreamProtocol := OpenAIUpstreamProtocolResponses
	...
	if account.IsOAuth() {
		...
		upstreamProtocol = account.ResolveOpenAIUpstreamProtocol()
		if upstreamProtocol == OpenAIUpstreamProtocolChatCompletions {
	...
	// Create OpenAI test payload matching the configured upstream protocol.
	payload := createOpenAITestPayload(testModelID, isOAuth)
	if !isOAuth && upstreamProtocol == OpenAIUpstreamProtocolChatCompletions {
	...
	if resp.StatusCode != http.StatusOK {
		...
		if s.accountRepo != nil {
			...
			case http.StatusUnauthorized:
				...
				default:
					if isOAuth {
						if strings.EqualFold(strings.TrimSpace(upstreamMsg), "unauthorized") {
