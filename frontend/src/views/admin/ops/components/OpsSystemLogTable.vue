...
const formatSystemLogDetail = (row: OpsSystemLog) => {
  ...
  const rectifierParts = rectifierKeys
    .map((key) => {
      const value = getExtraString(extra, key)
      return value ? `${key}=${value}` : ''
    })
    .filter(Boolean)
  if (rectifierParts.length > 0) parts.push(rectifierParts.join(' '))
...
const showInitialLoading = computed(() => loading.value && !hasData.value)
const showRefreshingState = computed(() => loading.value && hasData.value)
...
<template>
  <section class="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-dark-700 dark:bg-dark-900/60">
    ...
    <div class="relative overflow-hidden rounded-xl border border-gray-200 dark:border-dark-700">
      <div v-if="showInitialLoading" class="px-4 py-8 text-center text-sm text-gray-500">加载中...</div>
